...
def merge_caption_blocks(caption: base.Block, continuation: base.Block) -> None:
    ...
    # Keep the original bbox: the right-column continuation may begin near the page
    # top, but the source figure is still above the first caption fragment.
...
def primary_authors(doc, correspondence: str) -> list[str]:
    """Extract the compact primary byline rather than supplementary rosters.

    A true byline block is structurally dominated by ``Name + affiliation number``
    tokens. Reference lists and Methods prose can accidentally match the old name
    regex, but their match coverage is far lower. This density criterion is robust
    to publisher layouts where the byline is repeated at the end of the article.
    """
    ...
    for page_index in range(len(doc)):
        for _block_index, b in enumerate(doc[page_index].get_text("blocks", sort=False)):
            ...
            # Compact bylines are overwhelmingly affiliation-tagged names. A lower
            # threshold admits long multi-institution bylines, while still rejecting
            # prose/reference blocks observed at <0.25 in regression PDFs.
            if coverage < 0.48:
                continue
            ...
            # Recover collective authors that intentionally have no affiliation
            # number, then insert them in their source-order position.
            ...
            if groups:
                ...
                for _pos, name in ordered:
                    if name and name not in dedup:
                        dedup.append(name)
                # ALAST occasionally contributes a final author not consumed by
                # AFIRST; preserve it if needed.
...
# v16's build() resolves these functions dynamically from its own module; the final
# layout parser resolves its helpers dynamically from its module. Patch both layers
# before delegating to the established v16 entry point.
